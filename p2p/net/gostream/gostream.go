...
//
// 给定一个 libp2p.Host，gostream 提供 Dial() 和 Listen() 方法，这些方法返回 net.Conn 和 net.Listener 的实现。
//
// 与常规的 "host:port" 寻址不同，`gostream` 使用对等点 ID，并且不使用原始 TCP 连接，而是使用 libp2p 的 net.Stream。
// 这意味着您的连接将利用 LibP2P 的多路由、NAT 穿透和流多路复用功能。
//
// 注意，LibP2P 主机不能拨号到自身，因此无法将同一个 Host 同时用作服务器和客户端。
package gostream
