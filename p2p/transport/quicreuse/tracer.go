...
func (l *qlogger) Close() error {
	...
	if err != nil {
		...
		return fmt.Errorf("创建zstd编码器时出错: %w", err)
	}
	if _, err := io.Copy(c, l.f); err != nil {
		log.Errorf("复制qlog文件时出错: %s", err)
		return fmt.Errorf("复制qlog文件时出错: %w", err)
	}
	if err := c.Close(); err != nil {
		log.Errorf("关闭zstd编码器时出错: %s", err)
		return fmt.Errorf("关闭zstd编码器时出错: %w", err)
	}
	return buf.Flush()
}
